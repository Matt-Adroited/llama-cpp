...
// ============================================================================
// KV cache dequantization helper (v1: dense types only — F32, F16, BF16, Q8_0)
// ============================================================================
    ...
    uint32_t             n_kv_heads)
{
    ...
    for (uint32_t ci = 0; ci < n_cells; ci++) {
        ...
        float * dst = out + (size_t)ci * padded_hd;
        ...
        switch (k_type) {
            ...
            case GGML_TYPE_TQ3_0:
                dequantize_row_tq3_0((const block_tq3_0 *)quant_buf.data(), dst, padded_hd);
                break;
            case GGML_TYPE_TQ4_0:
                dequantize_row_tq4_0((const block_tq4_0 *)quant_buf.data(), dst, padded_hd);
                break;
            case GGML_TYPE_F16: {
            ...
            default:
                ...
                memset(dst, 0, padded_hd * sizeof(float));
                continue;
        }
    }
}
...
static void triattention_init_gpu(triattention_state * state, ggml_type k_type) {
    ...
    // Supported on GPU: dense (F32/F16/Q8_0) + TurboQuant (TQ3_0/TQ4_0, head_dim=128).
    if (k_type != GGML_TYPE_F32 && k_type != GGML_TYPE_F16 && k_type != GGML_TYPE_Q8_0 &&
        k_type != GGML_TYPE_TQ3_0 && k_type != GGML_TYPE_TQ4_0) {
        fprintf(stderr,
            "[TriAttention] GPU scoring unsupported for k_type=%d, using CPU\n", (int)k_type);
        return;
    }
    ...
    if ((k_type == GGML_TYPE_TQ3_0 || k_type == GGML_TYPE_TQ4_0) && cal->head_dim != 128) {
        fprintf(stderr,
            "[TriAttention] TQ3_0/TQ4_0 GPU path requires head_dim=128, got %u — using CPU\n",
            cal->head_dim);
        return;
    }
...
int32_t triattention_prune(
    ...
    llama_kv_cache     * kv)
{
    // Public entry point is unused in this build — real work happens in
    // triattention_prune_impl() called from llama_kv_cache integration.
    (void)kv;
    if (!state) return -1;
    return 0;
}
    ...
    uint32_t              kv_size)
{
    ...
    std::vector<uint32_t> decode_cell_idx;
